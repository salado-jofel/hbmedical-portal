# Meridian Portal — Codebase Patterns Reference
...
| `MeridianLogo`      | —                                                                                 | Meridian wordmark + mountain-peak mark                                            |
...
## SECTION R — Key Patterns Established in Commissions Feature

### Non-Blocking Webhook Side Effects

When a server action should run as a side effect of a Stripe webhook handler but must never block the payment flow:

```typescript
// In the webhook handler file (e.g. handle-checkout-webhook.ts):
async function calculateCommissionSafely(orderId: string) {
  try {
    await calculateOrderCommission(orderId);
  } catch (error) {
    console.error("[payments.calculateCommissionSafely] Commission calculation failed for order:", orderId, error);
  }
}

// Call AFTER all critical operations, inside the idempotency guard:
...
**Rules:**
- Always wrap in try/catch — never let side effects throw into the webhook handler
- Name the wrapper `<action>Safely` to signal non-blocking intent
- Log with a namespaced prefix: `[feature.helperName]`
- Call only inside an idempotency guard (`!wasAlreadyPaid && didMarkPaid`) to prevent duplicates

### Insert-First-Then-Close Versioning

When managing versioned records (e.g. commission rates where only one active record per entity is allowed):

```typescript
// 1. Insert the new record first — capture its ID
const { data: insertedRate, error: insertError } = await adminClient
  .from("commission_rates")
  .insert({ rep_id, rate_percent, effective_to: null, ... })
  .select("id")
  .single();
if (insertError) return { success: false, error: "Failed to save rate." };

// 2. Close all other active rates, excluding the one just inserted
await adminClient
  .from("commission_rates")
...
**Why insert-first:** If the insert fails, no data is lost — the prior active rate is still intact. Closing first and then failing on insert would leave the entity with no active rate.

### useState + useEffect for Redux-Sourced Initial Values

When a slider/input needs to initialize from Redux state that is null until `Providers.tsx` runs its `useEffect`:

```typescript
// ❌ WRONG — useState reads null on first render; never re-initializes when Redux hydrates
const [commRate, setCommRate] = useState(summary?.currentRate ?? 5);

// ✅ CORRECT — start with a safe default, sync via useEffect after hydration
...
**Why:** `useState` initializer runs once on mount. Redux state is null at that point because `Providers.tsx` dispatches in its own `useEffect` (which runs after all children mount). The `useEffect` sync fires after Redux hydrates and correctly updates the local state.
