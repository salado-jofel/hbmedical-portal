...
export function UserStatusChart({ users }: { users: Array<{ status: string }> }) {
  const { data, total } = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const u of users) {
      const key = u.status ?? "inactive";
      counts[key] = (counts[key] ?? 0) + 1;
    }
    const entries = STATUS_CONFIG.map((c) => ({
      label: c.label,
      count: counts[c.key] ?? 0,
      color: c.color,
    })).filter((e) => e.count > 0);
    return { data: entries, total: users.length };
  }, [users]);
  ...
  return (
    <div className="overflow-hidden rounded-[var(--r)] border border-[var(--border)] bg-[var(--surface)]">
      <div className="border-b border-[var(--border)] px-4 py-[0.8rem]">
        <p className="text-[13px] font-semibold text-[var(--navy)]">User Status</p>
        <p className="mt-[1px] text-[11px] text-[var(--text3)]">Team health overview</p>
      </div>
      <div className="p-4">
        {data.length === 0 ? (
