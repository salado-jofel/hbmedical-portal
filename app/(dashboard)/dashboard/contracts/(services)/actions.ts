...
export type PrepareContractUploadInput = {
  fileName: string;
  contentType: string;
};

export type PrepareContractUploadResult = {
  bucket: string;
  filePath: string;
  token: string;
};

export type CompleteContractUploadInput = {
  title: string;
  tag: string;
  bucket: string;
  filePath: string;
...
export async function prepareContractUpload(
  input: PrepareContractUploadInput,
): Promise<PrepareContractUploadResult> {
  const supabase = await createClient();
  ...
  const filePath = buildStoragePath(STORAGE_FOLDERS.contracts, uniqueFileName);
...
export async function completeContractUpload(
  input: CompleteContractUploadInput,
): Promise<void> {
  const supabase = await createClient();
  await requireAdminOrThrow(supabase);
  ...
  const { error: insertError } = await admin.from("contract_materials").insert({
    title,
    tag,
    bucket,
    file_path: filePath,
...
export async function bulkDeleteContractMaterials(
  ids: string[],
): Promise<void> {
