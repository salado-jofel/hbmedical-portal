...
import { Share2, Users, UserPlus, UserX, UserCheck, Loader2, Info, Mail, Trash2 } from "lucide-react";
...
const STATUS_CONFIG: Record<SubRepStatus, { bg: string; text: string; dot: string; label: string }> = {
...
export function OnboardingPageClient({
  ...
  const [clinicInviteState, clinicInviteAction, isClinicInvitePending] = useActionState<IInviteTokenFormState | null, FormData>(
    generateClinicMemberInvite,
    null,
  );
  ...
  async function handleStatusChange(subRepId: string, status: "active" | "inactive") {
    ...
    try {
      ...
      toast.success(status === "inactive" ? "Sub-rep disabled." : "Sub-rep reactivated.");
      router.refresh();
    } catch {
  ...
  async function handleResendSubRep(rep: ISubRep) {
    ...
    try {
      const result = await resendSubRepInvite(rep.id, rep.email, rep.first_name);
      if (!result.success) {
  ...
  const subRepColumns: TableColumn<ISubRep>[] = [
    {
      ...
      render: (rep) => (
        <div className="min-w-0">
          <p className={`text-sm font-medium truncate ${rep.status === "inactive" ? "text-[#94A3B8]" : "text-[#0F172A]"}`}>
      ...
      render: (rep) => {
        if (rep.status === "pending") {
          ...
          return (
            <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); handleResendSubRep(rep); }}
              ...
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); setDeleteSubRepId(rep.id); setDeleteSubRepConfirmOpen(true); }}
                disabled={isResending || isDeleting}
        ...
        if (rep.status === "active") {
          ...
          return (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); setDisableConfirmId(rep.id); }}
              disabled={isActing}
        ...
        // inactive
        const isThisLoading = (isUpdating && actionId === rep.id) || (isDeletingSubRep && deleteSubRepId === rep.id);
        return (
          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button
              ...
              onClick={(e) => { e.stopPropagation(); handleStatusChange(rep.id, "active"); }}
            >
              {isThisLoading
                ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-6">
      ...
      {showClinicStaffSection && (
        <section className="bg-white rounded-xl border border-[#E2E8F0] p-5 space-y-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-[#0F172A]">Invite Clinic Staff</h2>
          </div>
          <p className="text-sm text-[#64748B]">
            Generate a one-time link to invite a Clinical Staff member to your facility.
          </p>
          ...
          {clinicInviteState?.success && clinicInviteState.token ? (
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3" suppressHydrationWarning>
            ...
            <div className="space-y-2" suppressHydrationWarning>
              {tokens.map((token) => (
                <InviteTokenCard
                  ...
                  token={token}
                  baseUrl={baseUrl}
                  onDeleteClick={() => { setDeleteTokenId(token.id); setTokenConfirmOpen(true); }}
      ...
      <ConfirmModal
        ...
        onOpenChange={(open) => { if (!open) setDisableConfirmId(null); }}
        title="Deactivate Sub-Rep?"
        ...
        onConfirm={() => { if (disableConfirmId) handleStatusChange(disableConfirmId, "inactive"); }}
      />

      {/* ── Delete Sub-Rep Confirm ── */}
      <ConfirmModal
        open={deleteSubRepConfirmOpen}
        onOpenChange={(v) => { if (!isDeletingSubRep) { setDeleteSubRepConfirmOpen(v); if (!v) setDeleteSubRepId(null); } }}
        title="Delete Sub-Rep?"
      ...
      <ConfirmModal
        ...
        onOpenChange={(v) => { if (!isDeletingToken) { setTokenConfirmOpen(v); if (!v) setDeleteTokenId(null); } }}
