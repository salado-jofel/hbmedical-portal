...
function getHospitalOnboardingIcon(tag?: string | null) {
...
export default function HospitalOnboardingCards() {
  const items = useAppSelector((state) => state.hospitalOnboarding.items);
    ...
    <MaterialsGrid isEmpty={false}>
      {items.map((card) => (
        <MaterialCard
          ...
          icon={getHospitalOnboardingIcon(card.tag)}
        />
      ))}
    </MaterialsGrid>
  );
